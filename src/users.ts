...
export class UserManager {
    ...
    private static async saveUsers() {
        await fs.writeFile(USERS_FILE, JSON.stringify(this.users, null, 4), "utf-8");
    }

    static async saveSession(session: Session) {
        this.sessions.set(session.id, session);
        await fs.writeFile(path.join(SESSIONS_DIR, `${session.id}.json`), JSON.stringify(session, null, 4), "utf-8");
    }
